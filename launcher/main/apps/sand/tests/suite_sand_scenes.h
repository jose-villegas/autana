...
/* The layout set: boards painted to whatever grid the sand_t already carries,
 * so a chunk cut can be ranked at every quality. Shared because the host
 * pre-filter and the emulated sweep must rank the board they measure. Each
 * returns the steps to run under the measured gravity first - 0 where the
 * transient is the point. The last two are the gas pair. */
...
int build_layout_gas_column_scene(sand_t* s);
int build_layout_gas_box_scene(sand_t* s);
