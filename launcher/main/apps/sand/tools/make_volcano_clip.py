...
def extract_frames(video: Path, work: Path) -> list[Path]:
    subprocess.run(
        [
            ...
            "-vf", "select=not(mod(n\\,3))", "-vsync", "0", str(work / "frame-%03d.png"),
        ],
        check=True,
    )
...
def composite(panel: Image.Image, angle: float) -> Image.Image:
    ...
    plate = Image.new("RGBA", (440, 366), "#080d13")
    plate.paste(panel.resize((412, 338), Image.Resampling.LANCZOS).convert("RGBA"), (14, 14))
    spun = plate.rotate(-angle, Image.Resampling.BICUBIC, expand=True)
    canvas.paste(spun, ((size - spun.width) // 2, (size - spun.height) // 2), spun)
    return canvas.resize((280, 280), Image.Resampling.LANCZOS)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        images = [composite(Image.open(path), angles[i * 3]) for i, path in enumerate(paths)]
        if args.contact:
            contact = Image.new("RGB", (840, 560))
            for slot, index in enumerate((0, 9, 18, 27, 36, 49)):
                contact.paste(images[index], ((slot % 3) * 280, (slot // 3) * 280))
