 ...
 * touch controller and IMU that CONFIG_LAUNCHER_QEMU has neither of.
 *
 ...
 * real controller answers there is no stand-in to write to, which is why
 * this follows those functions' own gating rather than the listener's.
 */
...
console_verb_imu(const char* args, console_reply_fn reply) {
    (void)reply;
    int ax = 0, ay = 0, az = 0;
    if (console_imu_parse(args, &ax, &ay, &az)) {
        imu_inject(&(imu_sample_t){.ax = (int16_t)ax, .ay = (int16_t)ay, .az = (int16_t)az});
    } else {
        ESP_LOGW(TAG, "IMU wants <ax> <ay> <az> in raw counts: '%s'", args);
    }
}
