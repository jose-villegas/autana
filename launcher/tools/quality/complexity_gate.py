...
def build_tools_entries(host_flags, already_covered):
    ...
    for path in sorted((LAUNCHER_DIR / "main").glob("apps/*/tools/*.c")):
        ...
        render_include = (LAUNCHER_DIR / "tools" / "render").as_posix()
        entries[resolved] = (["clang"] + host_flags +
                              ["-I", extra_include, "-I", render_include,
...
def build_compile_db():
    ...
    if host_target is None:
        sys.exit(
            "Could not resolve a host compiler via tools/build/find_cc.sh - "
...
def write_baseline(path, scores):
    lines = [
        "# generated by launcher/tools/quality/complexity_gate.py --update-baseline",
