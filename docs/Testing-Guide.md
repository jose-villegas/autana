...
| render lab | `run_cube_perf_suite`, `run_cube_band_perf_suite`, `run_small3dlib_scissor_suite`, `run_wire_pipeline_suite`, `run_wire_primitives_suite` | the cube scene's frame budget, band mode against the full framebuffer across orientations, the rasterizer's row scissor; the wireframe transform/near-clip/screen-clip pipeline and the baked plane/cube/sphere/capsule edge lists it draws |
