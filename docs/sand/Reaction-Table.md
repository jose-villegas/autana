...
Typography carries the other three roles colour does not: *italic* is the verb driving a clause, **bold** is a rate or frequency word (silent, rather than printed, for the common case - see the generator's own adverb_for()), and ***bold italic*** is a trigger clause that lives at a read site in sand_reactions.c rather than in the table itself, recovered from that file's own REACTION_DOC() annotation at the point that decides it rather than guessed at.
...
Generated by walking the RATE field that drives each reaction and branching on its target, never by walking `*_to` fields - see the generator's own top comment on the pairwise join for why (`soaks_to == 0` is a real reaction with no target, and iterating `*_to` alone would silently skip it).
