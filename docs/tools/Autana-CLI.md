...
| `autana reset [--capture [seconds]] [--verbose]` | Reboot the board and wait for its USB serial port to return. `--capture` records the boot console and prints its record path and error, panic, abort, or assert lines, for 20 seconds when omitted; what the board prints while USB re-enumerates may be lost. `--verbose` prints the full capture. |
| `autana suite <name> [seconds] [--verbose]` | Run one registered suite. Prints the report path, PASS/FAIL counts, failure messages, and capture end reason. Failure output is capped at ten lines with a pointer to the record for the rest. `--verbose` prints the full console capture. |
| `autana suite list [text]` | The suites this worktree registers, read from its sources; `[text]` keeps the names containing it. |
| `autana selftest [seconds] [--verbose]` | Build the diagnostics+autorun image and run every suite this worktree registers, on the device - 3000 seconds when omitted; can take minutes. Prints the report path, PASS/FAIL counts, up to ten failure messages, and capture end reason. `--verbose` prints the full console capture. |
| `autana batch <suite> [<suite> ...] [--runs N] [--perf-scope] [--verbose]` | Flash the diagnostics image once and capture the given suites `--runs` times (3 when omitted) under one lock, so no other session can flash between two captures of the same image; writes one summary across every run. Each run prints its report path, PASS/FAIL counts, up to ten failure messages, and capture end reason. `--verbose` prints each full console capture. |
