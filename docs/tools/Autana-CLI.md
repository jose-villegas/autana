...
| `autana reset [--capture [seconds]]` | Reboot the board and wait for its USB serial port to return. `--capture` prints and records the boot console for 20 seconds when no window is given; the record says how it ended and that bytes may have been lost while USB re-enumerated. |
